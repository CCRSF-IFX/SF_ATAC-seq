...
rule genrich:
    ...
    shell: """
        ...
        cut -f 1,2,3,4 {output.bdg} | grep -v 'chrUn' | grep -v 'NW' | tail -n +2 > {params.prefix}.bedgraph
        """
...
rule Bdg2Bw:
    ...
    shell: "sort -k1,1 -k2,2n -T ./ --parallel = {threads} {input.bedgraph} > {params.prefix}.tmp.bedgraph; bedGraphToBigWig {params.prefix}.tmp.bedgraph {params.sizes} {output.bigwig}"
...
rule plotFingerprint:
    ...
    shell: """deeptools; plotFingerprint --numberOfProcessors {threads} -b {params.bam} --labels {params.labels} --minMappingQuality 5 --skipZeros --ignoreDuplicates --outQualityMetrics plotFingerprint_QC_metrics.txt -T "Fingerprints of ATAC-seq samples" --plotFile {output.png} --outQualityMetrics {params.prefix}.tab --outRawCounts Deeptools/RawCounts.tab"""
...
rule bigwig_summary:
    ...
    threads: 8
    shell: "deeptools; multiBigwigSummary BED-file -p 8 -b {params.bigwig} --labels {params.labels} -o {output.bw} --BED {params.bed}"
...
rule plotCorrelation:
    ...
    threads: 8
    shell: """
        deeptools; plotCorrelation -in {input.bw} --corMethod spearman --skipZeros --whatToPlot heatmap -T "Spearman Correlation of ATAC-seq samples" -o {output.spearman} --outFileCorMatrix Deeptools/SpearmanCor_bigwigScores.tab
        deeptools; plotCorrelation -in {input.bw} --corMethod pearson --skipZeros --whatToPlot heatmap -T "Pearson Correlation of ATAC-seq samples" -o {output.pearson} --outFileCorMatrix Deeptools/PearsonCor_bigwigScores.tab
        ...
        deeptools; computeMatrix reference-point -p 16 -a 1000 -b 1000 --referencePoint TSS -S {params.bigwig} -R {params.bed} --skipZeros -o {output.computeMat} --samplesLabel {params.labels}
        deeptools; plotHeatmap -m {output.computeMat} -out {output.heatmap} --dpi 100
        deeptools; plotProfile -m {output.computeMat} -out {output.profile} --dpi 100
       """

rule alignmentSieve:
    input: sortBam = "Sample_{sample}/{sample}.sorted.markdup.bam", sortBai = "Sample_{sample}/{sample}.sorted.markdup.bam.bai"
    output: bam = "Sample_{sample}/{sample}.sorted.markdup.filtered.bam", sort = "Sample_{sample}/{sample}.sorted.markdup.filtered.sort.bam"
    params: smp = "{sample}"
    shell: """
        deeptools; alignmentSieve -p 8 -b {input.sortBam} -o {output.bam} -l {params.smp} --ATACshift --ignoreDuplicates --minMappingQuality 5 --minFragmentLength 20
...
if len(samples)> 2:
    rule plotPCA:
        ...
        shell: "deeptools}; plotPCA -in {input} -o {output} --outFileNameData Deeptools/PCA.tab"
...
rule bamPEFragmentSize:
    ...
    shell: "deeptools; bamPEFragmentSize -T "Fragment size of ATAC-seq data" --maxFragmentLength 1000 -b {params.bamFiles} --samplesLabel {params.labels} --outRawFragmentLengths Deeptools/outRawFragmentLengths.txt --table Deeptools/table.txt"
...
rule featureCounts:
    ...
    output: fc = "Genrich/{sample}.genrich.fc.txt", err = "Genrich/{sample}.genrich.fc.err", log ="Genrich/{sample}.genrich.fc.log"
    conda: "envs/featureCounts.yaml"
    log: "logs/featureCounts.log"
    threads: 8
    shell:"featureCounts -p -T {threads} -a {input.saf} -F SAF -o {output.fc} {input.bam}  2>{output.err} 1>{output.log}"
