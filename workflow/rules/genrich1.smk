...
rule featureCounts:
    ...
    output: fc = "Genrich/{sample}.genrich.fc.txt"
    conda: "envs/featureCounts.yaml"
    log: "logs/{sample}.featureCounts.log"
    threads: 8
    shell:"featureCounts -p -T {threads} -a {input.saf} -F SAF -o {output.fc} {input.bam}"

rule fripScore:
    input: "logs/{sample}.featureCounts.log"
...
rule ChIPseeker:
    ...
    conda: "envs/R.yaml"
    log: "logs/bamPEFragmentSize.log"
    shell: "RScript ChIPseeker {params.ref} {input}"
